package prcontigfilter::prcontigfilterClient;
...
prcontigfilter::prcontigfilterClient

=head1 DESCRIPTION


A KBase module: prcontigfilter
...
sub new
{
    ...
    my $self = {
	client => prcontigfilter::prcontigfilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a prcontigfilter.FilterContigsParams
$return is a prcontigfilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a prcontigfilter.workspace_name
	contigset_id has a value which is a prcontigfilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a prcontigfilter.ws_contigset_id
...
# Authentication: required
	...
	method => "prcontigfilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for prcontigfilter::prcontigfilterClient\n";
    }
    if ($sMajor == 0) {
        warn "prcontigfilter::prcontigfilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package prcontigfilter::prcontigfilterClient::RpcClient;
